...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    iter_end = torch.cuda.Event(enable_timing = True) # 用于计算每个iteration的时间
    ...
    for iteration in range(first_iter, opt.iterations + 1):   # 测试与网络gui交互      
        ...
        if (iteration - 1) == debug_from:
            pipe.debug = True
        render_pkg = render(viewpoint_cam, gaussians,dataset.white_background) # 调用gaussian_renderer中的render函数进行光栅化渲染
        image, viewspace_point_tensor, visibility_filter, radii = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"]

        viewspace_point_tensor.retain_grad() # 保留视空间坐标的梯度
        ...
        non_zero_row_indices = torch.any(grad != 0, axis=1)
        non_zero_rows = grad[non_zero_row_indices]
        # print(non_zero_rows)
        viewspace_point_tensor_grad = torch.cat([grad, torch.zeros((grad.shape[0], 1), dtype=grad.dtype).cuda()], dim=1) # 由于视空间坐标是三维的，而梯度是二维的，所以需要在梯度后面加一个0
        viewspace_point_tensor_grad = torch.tensor(viewspace_point_tensor_grad, dtype=viewspace_point_tensor.dtype) # 转换为tensor
        ...
        with torch.no_grad(): 
            ...
            # Log and save
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render,dataset.white_background)
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    image = torch.clamp(renderFunc(viewpoint, scene.gaussians, white_background)["render"], 0.0, 1.0)
