...
class GaussianModel: # 定义Gaussian模型，初始化与Gaussian模型相关的各种属性，调用setup_functions方法设置各种激活和变换函数
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        fused_color = RGB2SH(torch.tensor(np.asarray(pcd.colors)).float().cuda()) #将输入点云数据的颜色信息转换为球谐函数表示，并移动到GPU上
        features = torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().cuda() #创建一个零张量，用于存储特征信息，其形状为 (点的数量, 3, 球谐函数的维度)
        ...
        opacities = inverse_sigmoid(0.1 * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda")) # 创建一个张量，用于存储点的不透明度信息，其形状为 (点的数量, 1)，并将其初始化为0.1
    ...
    def training_setup(self, training_args): # 该方法用于设置训练参数和优化器
        ...
        self.denom = torch.zeros((self.get_xyz.shape[0], 1), device="cuda") # 创建两个零张量，用于存储点云中每个点的梯度累积和梯度累积次数，其形状都为 (点的数量, 1)
        ...
        self.optimizer = torch.optim.Adam(l, lr=0.0, eps=1e-15) # 创建一个Adam优化器，用于优化上面的参数列表
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation): #  用于在密集化处理后更新高斯模型的参数。
        ...
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda") # 重置梯度累积和梯度累积次数，以及最大2D半径便于后续密集化和修剪
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        selected_pts_mask = torch.logical_and(selected_pts_mask,
                                              torch.max(self.get_scaling, dim=1).values > self.percent_dense*scene_extent) # 生成掩码
